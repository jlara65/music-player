  ...
  $.ajax({
    ...
    $.each(content, (index, album) => {
      const $album = $("<span>")
        .text(album?.album)
        .append(
          `<img class="flex justify- rounded m-0" src="${album?.image}" width="75%" height="75%"/>`
        );
...
function getArtistTrack(artistName) {
  $.ajax({
    ...
    tracks.forEach((track) => {
      ...
      const $track = $("<h3>").text(data?.artists?.items[0]?.profile?.name)
        .append(`
...
>>>>>>> 253727fdc95bd116968379550c2fae1cab6d3be0
  ...
  for (var artist of searchHistory) {
    var searchHistoryItem = $(
      '<button type="button" class="search-history-item bg-emerald-50 h-box"">'
    );
    searchHistoryItem.text(artist).data("artist", artist);
    $(".search-history").append(searchHistoryItem);
  }
